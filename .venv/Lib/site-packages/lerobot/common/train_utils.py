...
def update_last_checkpoint(checkpoint_dir):
    ...
    if last_checkpoint_dir.is_symlink() or last_checkpoint_dir.exists():
        if last_checkpoint_dir.is_dir() and not last_checkpoint_dir.is_symlink():
            shutil.rmtree(last_checkpoint_dir)
        else:
            last_checkpoint_dir.unlink()
    try:
        last_checkpoint_dir.symlink_to(checkpoint_dir.relative_to(last_checkpoint_dir.parent), target_is_directory=True)
    except OSError:
        # Fallback for Windows standard users without symlink privileges
